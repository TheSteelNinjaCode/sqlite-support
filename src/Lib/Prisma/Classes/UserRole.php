...
class UserRole implements IModel
{
    ...
    protected function includeUser(array $items, array $selectedFields = [], array $includeSelectedFields = []) 
    {
        ...
        $modelName = new User($this->pdo);
        foreach ($items as &$item) {
            if (!isset($item[$primaryKey])) {
                $item[$relationName] = [];
                continue;
            }
            $relatedRecords = $modelName->findUnique(['where' => [$foreignKey => $item[$primaryKey]], 'select' => $includeSelectedFields]);
    ...
    protected function connectUser(string $relationName, array $connectData, string $lastInsertId, string $connectType = 'connect')
    {
        ...
        $relationModel = new User($this->pdo);
    ...
    public function findUnique(array $criteria, string $format = 'array'): array | object
    {
        ...
        if ($format === 'object') {
            return new UserRole($this->pdo, $record);
        }
    ...
    public function upsert(array $data, string $format = 'array'): array | object
    {
        ...
        Utility::checkForInvalidKeys($data, $acceptedCriteria, 'UserRole');
